...
RUN mkdir -p /out/work /out/registry-data && \
	...
	CGO_ENABLED=0 GOPROXY=off GOSUMDB=off go build -mod=vendor -trimpath -buildvcs=true -ldflags "-s -w" -o /out/registry ./cmd/registry && \
	CGO_ENABLED=0 GOPROXY=off GOSUMDB=off go build -mod=vendor -trimpath -buildvcs=true -ldflags "-s -w" -o /out/reference-server ./cmd/reference-server
