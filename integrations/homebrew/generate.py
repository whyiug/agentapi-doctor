#!/usr/bin/env python3
"""Render a Homebrew formula only from an exact version and local checksums."""
...
OUTPUT_NAME = "agentapi-doctor.rb"
...
def render_template(template: str, values: Mapping[str, str]) -> str:
    ...
    if observed != set(values):
        raise validate.ValidationError("formula template has missing or unknown tokens")
    rendered = TOKEN_RE.sub(lambda match: values[match.group(1)], template)
    if "{{" in rendered or "}}" in rendered:
        raise validate.ValidationError("formula template left unresolved tokens")
    return rendered
...
def write_exclusive(path: Path, content: str) -> None:
    ...
    if not encoded or len(encoded) > validate.MAX_FORMULA_BYTES:
        raise validate.ValidationError("rendered formula violates its byte limit")
...
def generate(version: str, checksums_path: Path, output_directory: Path) -> Path:
    ...
    template_path = Path(__file__).with_name("agentapi-doctor.rb.tmpl")
    try:
        template = validate.read_regular(template_path, validate.MAX_FORMULA_BYTES).decode("utf-8")
    except UnicodeDecodeError as error:
        raise validate.ValidationError("formula template must be UTF-8") from error
    ...
    validate.validate_rendered_formula(destination, version, checksums_path)
    return destination
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"homebrew generator: {error}", file=sys.stderr)
        return 2
    print(destination)
    return 0
